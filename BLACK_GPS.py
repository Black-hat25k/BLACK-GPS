...
def logo():
 ...
 /  _____/\______   \/   _____/  %sCoded by BLACK HAT 25%s
/   \  ___ |     ___/\_____  \   %sThx to D4RKSH4D0WS%s
\    \_\  \|    |    /        \  %sTEAM BLACK MINIONS 25%s
 \______  /|____|   /_______  /  TRACKER
        \/                  \/
	'''%(C,W,C,W,C,W,C)
def gps(link,id):
	print '\n%s[%s!%s] kirim link ke target: %s'%(W,R,W,link)
	print '%s[%s!%s] menunggu target, press ctrl + c to exit ...\n'%(W,R,W)
	res_loc=[]
	res_ip=[]
	res_ua=[]
	while True:
		try:
			...
			for arz in track:
				...
				else:res_loc.append(loc);print '%s[%s✓%s] lokasi: %s\n'%(W,G,W,loc)
				if ip in res_ip:continue
				else:res_ip.append(ip);print '%s[%s✓%s] IP : %s\n'%(W,G,W,ip)
				if ua in res_ua:continue
				else:res_ua.append(ua);print '%s[%s✓%s] info hp: %s\n'%(W,G,W,ua)
